...
export async function rmtMonitor2(msg: Message) {
    ...
    let cooldown = 0;
    try {
        const cooldownPostgres = await pool.query('SELECT date FROM cooldown2 WHERE channelID = $1 AND identifier = $2', [msg.channelId, identifier]);
    ...
    try {
        ...
        if (msg.channelId == '1059657287293222912' || msg.channelId == '1060037469472555028' || msg.channelId == '1060623647649308712') {
            const ratersPostgres = await pool.query('SELECT meta, userid FROM raters WHERE channelID = $1 AND meta = $2', [msg.channelId, identifier]);
            ratersdbmatches = ratersPostgres.rows;
        }
        else {
            const ratersPostgres = await pool.query('SELECT meta, userid FROM raters WHERE channelID = $1 AND gen = $2', [msg.channelId, identifier]);
            ratersdbmatches = ratersPostgres.rows;
        }
    ...
    try {
        if (cooldown === 0) {
            await pool.query('INSERT INTO cooldown2 (channelid, identifier) VALUES ($1, $2)', [msg.channelId, identifier]);    
        }
        else {
            await pool.query('UPDATE cooldown2 SET date = default WHERE channelid = $1 AND identifier = $2', [msg.channelId, identifier]);
        }
