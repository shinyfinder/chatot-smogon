        ...
        if (profileURL === null) {
            ...
            if (typeof forumid === 'string') {
                ...
                const [sqlMatch] = await sqlPool.execute(`
                    ...
                    FROM xenforo.xf_user 
                    INNER JOIN xenforo.xf_user_field_value ON xenforo.xf_user.user_id = xenforo.xf_user_field_value.user_id 
                    WHERE xenforo.xf_user.username = BINARY ? AND field_id = "discord"`, [profileURL]);
            ...
            if (userData[0].field_value !== interaction.user.tag) {
                await interaction.followUp('Your forum account does not have your Discord tag in your Identities. Did you enter your username correctly?');
                return;
            }
